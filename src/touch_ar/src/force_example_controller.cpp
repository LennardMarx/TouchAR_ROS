...
bool ForceExampleController::init(hardware_interface::RobotHW *robot_hw,
  ...
  std::cout << "Touch AR force controller launched!" << std::endl;
  std::vector<std::string> joint_names;
  std::string arm_id;
  ROS_WARN("ForceExampleController: Make sure your robot's endeffector is in "
           "contact "
  ...
  if (!node_handle.getParam("joint_names", joint_names) ||
      joint_names.size() != 7) {
    ROS_ERROR("ForceExampleController: Invalid or no joint_names parameters "
              "provided, aborting "
              "controller init!");
    return false;
  }
  ...
  try {
    ...
    ROS_ERROR_STREAM("ForceExampleController: Exception getting model handle "
                     "from interface: "
                     << ex.what());
    return false;
  }
  ...
  try {
    ...
    ROS_ERROR_STREAM("ForceExampleController: Exception getting state handle "
                     "from interface: "
                     << ex.what());
    return false;
  }
  ...
  if (effort_joint_interface == nullptr) {
    ROS_ERROR_STREAM("ForceExampleController: Error getting effort joint "
  ...
  for (size_t i = 0; i < 7; ++i) {
    try {
      ...
      ROS_ERROR_STREAM(
          "ForceExampleController: Exception getting joint handles: "
          << ex.what());
      return false;
    }
  }
  ...
  dynamic_server_desired_mass_param_ =
      std::make_unique<dynamic_reconfigure::Server<
          franka_example_controllers::desired_mass_paramConfig>>(

          dynamic_reconfigure_desired_mass_param_node_);
  dynamic_server_desired_mass_param_->setCallback(boost::bind(
      &ForceExampleController::desiredMassParamCallback, this, _1, _2));
...
void ForceExampleController::update(const ros::Time & /*time*/,
  ...
  // desired_force_torque(2) = desired_mass_ * -4.81;
  ...
  desired_mass_ =
      filter_gain_ * target_mass_ + (1 - filter_gain_) * desired_mass_;
